      ...
      <div className="space-y-3 sm:space-y-4">
        {activities.map((activity, i) => (
          <motion.div
            ...
            whileHover={{ x: 2 }}
            className="flex items-start gap-3 sm:gap-4 pb-3 sm:pb-4 border-b border-border last:border-b-0 last:pb-0 cursor-pointer"
          >
            {/* Icon */}
            <motion.div
              whileHover={{ scale: 1.1, rotate: -10 }}
              className="w-8 sm:w-10 h-8 sm:h-10 rounded-lg bg-muted flex items-center justify-center flex-shrink-0 mt-0.5"
            >
              <Clock className="w-3 sm:w-4 h-3 sm:h-4 text-muted-foreground" />
            </motion.div>

            {/* Content */}
            <div className="flex-1 min-w-0">
              <p className="font-medium text-foreground text-xs sm:text-sm">{activity.title}</p>
